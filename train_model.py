...
def train_model(args):
    ...
    latest_run = max(runs_dir.glob(f'{args.name}*'), key=os.path.getctime) if runs_dir.exists() else None
    ...
    if latest_run:
        ...
        # Copy best.pt to current directory for easy access
        import shutil
        if best_pt.exists():
            shutil.copy2(best_pt, 'best.pt')
            print(f"✓ Copied best.pt to current directory")
        ...
        print("Warning: Could not find training output directory")
